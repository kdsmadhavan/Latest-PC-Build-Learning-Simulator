...
(function () {
    ...
    function navigateTo(page) {
        ...
        // Initialize simulator scene when entering simulator
        if (page === 'simulator' && simulator) {
            simulator.initScene();
            setTimeout(() => simulator.onResize(), 100);
        }
    ...
    function initSimulatorUI() {
        ...
        if (list) {
            list.innerHTML = simulator.components.map(c => `
                <div class="comp-slot" data-id="${c.id}" id="slot-${c.id}">
    ...
    function handleComponentClick(id) {
        ...
        simulator.installComponent(id, () => {
            ...
            if (slot) {
                ...
                slot.querySelector('.comp-slot-info span').textContent = 'Installed ✓';
            }
    ...
    function resetSimulator() {
        ...
        document.querySelectorAll('.comp-slot').forEach(s => {
            s.classList.remove('installed', 'active');
            s.querySelector('.comp-slot-info span').textContent = 'Click to install';
        });
        const hint = document.getElementById('sim-viewport-hint');
